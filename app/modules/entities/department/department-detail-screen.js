...
function DepartmentDetailScreen(props) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.paddedScrollView} testID="departmentDetailScrollView">
      ...
      {/* DepartmentPic Field */}
      <Text style={styles.label}>DepartmentPic:</Text>
      <Text testID="departmentPicContentType">{department.departmentPicContentType}</Text>
      <Image
        testID="departmentPic"
        style={styles.imageBlob}
        source={{ uri: `data:${department.departmentPicContentType};base64,${department.departmentPic}` }}
      />
      {/* DepartmentDescription Field */}
      <Text style={styles.label}>DepartmentDescription:</Text>
      <Text testID="departmentDescription">{department.departmentDescription}</Text>
      {/* DepartmentInfoPdf Field */}
      <Text style={styles.label}>DepartmentInfoPdf:</Text>
      <Text testID="departmentInfoPdf">Open {department.departmentInfoPdfContentType} (not implemented)</Text>
